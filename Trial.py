...
dlno = input("Enter DL number (format : DL-xxxxxxxxxxxxx): ")
...
url = get_url('https://parivahan.gov.in/rcdlstatus/?pur_cd=101')

jsession = url.cookies.get_dict().get('JSESSIONID')
...
def get_captcha(session_id):
    ...
    captcha_img = requests.get('https://parivahan.gov.in/rcdlstatus/DispplayCaptcha',headers=headers, params=params, cookies=cookies)

    img = Image.open(BytesIO(captcha_img.content))
    img.show()
    captcha_image = 'captcha_img_data.png'
    img.save(captcha_image)
...
def send_post(session_id, dl_number, birth_date, user_state, captcha_val):
    cookies = {
        'JSESSIONID': session_id,
        'has_js': '1',
    }
    ...
    response = requests.post('https://parivahan.gov.in/rcdlstatus/vahan/rcDlHome.xhtml',headers=headers, cookies=cookies, data=data)
    return response
...
def to_json(html_data):
    ...
    dom = etree.HTML(html)
    div = dom.xpath('//*[@id="form_rcdl:j_idt62"]')[0]
